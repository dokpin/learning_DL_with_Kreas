...
model2.add(layers.Dense(16, kernel_regularizer=regularizers.l1(0.001),
...
L1regularized_history = model2.fit(partial_x_train, partial_y_train,
							 epochs=20, batch_size=512, validation_data=(x_val, y_val))

L1regularized_history_dict = L1regularized_history.history
L1regularized_val_loss = L1regularized_history_dict['val_loss']
...
epochs = range(1, len(L1regularized_val_loss) + 1)

plt.plot(epochs, origin_val_loss, 'P', label='Original model')
plt.plot(epochs, L1regularized_val_loss, 'bo', label='L1-regularized model')
