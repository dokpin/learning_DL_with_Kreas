...
origin_training_loss = origin_history_dict['loss']

###

model2 = models.Sequential()
model2.add(layers.Dense(1024, activation='relu', input_shape=(10000,)))
...
bigger_history = model2.fit(partial_x_train, partial_y_train,
							 epochs=20, batch_size=512, validation_data=(x_val, y_val))

bigger_history_dict = bigger_history.history
bigger_val_loss = bigger_history_dict['val_loss']
bigger_training_loss = bigger_history_dict['loss']
...
epochs = range(1, len(bigger_val_loss) + 1)
...
plt.plot(epochs, origin_training_loss, 'P', label='Original model')
plt.plot(epochs, bigger_training_loss, 'bo', label='Bigger model')
