...
L2regularized_history = model2.fit(partial_x_train, partial_y_train,
							 epochs=20, batch_size=512, validation_data=(x_val, y_val))

L2regularized_history_dict = L2regularized_history.history
L2regularized_val_loss = L2regularized_history_dict['val_loss']
...
epochs = range(1, len(L2regularized_val_loss) + 1)

plt.plot(epochs, origin_val_loss, 'P', label='Original model')
plt.plot(epochs, L2regularized_val_loss, 'bo', label='L2-regularized model')
